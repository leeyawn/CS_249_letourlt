...
public class Test_CharBoard {
    ...
    public Object[][] mapGen() {
        ...
        for(int rows = 1; rows <= numRows; rows++) {
            for(int cols = 1; cols <= numCols; cols++) {
                maps[index][0] = new CharBoard(rows, cols, defaultChar);
                maps[index][1] = rows;
                maps[index][2] = cols;
    ...
    public void test_getRowCnt(CharBoard myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
        Assert.assertEquals(myMap.getRowCnt(), rowCnt.intValue());        
    }

    @Test(dataProvider = "mapGen")
    public void test_getColCnt(CharBoard myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
        Assert.assertEquals(myMap.getColCnt(), colCnt.intValue());        
    }

    @Test(dataProvider = "mapGen")
    public void test_isValidPosition(CharBoard myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
        // Check negative
    ...
    @Test(dataProvider = "mapGen")
    public void test_GetAndSetPos(CharBoard myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
